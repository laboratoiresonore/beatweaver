  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-1.5" title={connected ? deviceName : 'No MIDI controller'}>
        <div className={`w-2 h-2 rounded-full ${connected ? 'bg-green-400' : 'bg-dj-border'}`} />
        <span className="text-[10px] text-dj-muted uppercase">
      ...
      <div className="relative z-[999]" ref={panelRef}>
        <div className="flex items-center gap-0">
          ...
          <button
            ...
            className={`px-2 py-0.5 rounded-l text-[10px] font-bold transition-all ${
              announcerEnabled
                ? 'bg-dj-surface border border-green-600 text-green-400 border-r-0'
                : 'bg-dj-surface border border-dj-border text-dj-muted border-r-0'
            }`}
          ...
          <button
            ...
            className={`px-1 py-0.5 rounded-r text-[10px] font-bold transition-all ${
              showVoicePanel
                ...
                : announcerEnabled
                  ? 'bg-dj-surface border border-green-600 text-green-400'
                  : 'bg-dj-surface border border-dj-border text-dj-muted'
            }`}
